...
def gameLoop(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        snake_object = {"x1": x1, "y1": y1, "x1_change": random.choice(starts), "y1_change": random.choice(starts)}
    ...
    while not game_over and len(snakes_genomes) > 0:
        ...
        # When playing
        for x, snake in enumerate(snakes_genomes):  # give each snake a fitness of 0.01 for each frame it stays alive
            # ge[x].fitness += 0.01
            output = nets[x].activate((snake["x1"],snake["y1"], foodx, foody))
            if output[0] > 0 and snake["y1"] != 0:  # snake x go left
                snake["x1_change"] = -snake_block
                snake["y1_change"] = 0
            if output[0] < 0 and snake["y1"] != 0: # snake x go right
                snake["x1_change"] = snake_block
                snake["y1_change"] = 0
            if output[1] < 0 and snake["x1"] != 0: # snake x go up
                snake["y1_change"] = -snake_block
                snake["x1_change"] = 0
            if output[1] > 0 and snake["x1"] != 0: # snake x go down
                snake["y1_change"] = snake_block
                snake["x1_change"] = 0
        # Makes snake move on click
        for event in pygame.event.get():
        ...
        for x, snake in enumerate(snakes_genomes):
            ...
            if snake["x1"] >= dis_width or snake["x1"] < 0 or snake["y1"] >= dis_height or snake["y1"] < 0:
                ...
                # print(f"Fjerner {snake} da den treffer veggen")
            ...
            Your_score(Length_of_snake - 1)
            pygame.display.update()
            if snake["x1"] == foodx and snake["y1"] == foody: #Check if someone wins and add new "food"
                foodx = round(random.randrange(0, dis_width - snake_block) / 10.0) * 10.0
                foody = round(random.randrange(0, dis_height - snake_block) / 10.0) * 10.0
                Length_of_snake += 1
                ge[x].fitness +=5
